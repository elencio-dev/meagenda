...
export async function GET(request: Request) {
  ...
  try {
    ...
    for (const agendamento of agendamentos) {
      // Aqui integraria envio real via Resend / NodeMailer / Z-API.
      // Versão KISS via Log:
      console.log(`[LEMBRETE] Email/Zap enviado para ${agendamento.cliente.email || agendamento.cliente.phone} - Lembrete do agendamento dia ${new Date(agendamento.date).toLocaleDateString("pt-BR")} às ${agendamento.time} na empresa ${agendamento.empresa.name}.`)
