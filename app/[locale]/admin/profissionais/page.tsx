    ...
    try {
      if (editItem) {
        await fetch("/api/profissionais", {
          ...
          body: JSON.stringify({ id: editItem.id, name: form.name, role: form.role }),
        })
      } else {
        await fetch("/api/profissionais", {
          method: "POST",
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-sans text-3xl text-[var(--ink)]">Profissionais</h1>
          <p className="text-sm text-[var(--ink-60)] mt-1">{profissionais.length} profissionais cadastrados</p>
        </div>
          ...
          <DialogContent className="sm:max-w-md">
            ...
            <div className="space-y-4 pt-2">
              <div className="space-y-2">
                <label className="text-sm font-medium text-[var(--ink)]">Nome completo</label>
                <input
                  type="text"
                  value={form.name}
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-[var(--ink)]">Cargo / Especialidade</label>
                <input
                  type="text"
                  value={form.role}
                  onChange={e => setForm(f => ({ ...f, role: e.target.value }))}
                  placeholder="Ex: Cabeleireira, Manicure, Médico..."
                  className="w-full px-4 py-3 rounded-xl border border-[var(--ink-10)] bg-[var(--paper)] text-[var(--ink)] outline-none focus:border-[var(--coral)] focus:ring-2 focus:ring-[var(--coral)]/10 placeholder:text-[var(--ink-30)]"
                />
              </div>
              <div className="flex gap-3 pt-2">
                ...
                <Button
                  className="flex-1 rounded-xl bg-[var(--coral)] hover:bg-[var(--coral-dark)] text-white"
                  onClick={handleSave}
                  disabled={saving || !form.name.trim() || !form.role.trim()}
                >
                  {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : editItem ? "Salvar" : "Adicionar"}
                </Button>
              </div>
            </div>
          </DialogContent>
        ...
        <Card className="border-dashed border-[var(--ink-10)]">
          <CardContent className="flex flex-col items-center justify-center py-16 gap-4">
            ...
            <div className="text-center">
              <h3 className="font-medium text-[var(--ink)] mb-1">Nenhum profissional cadastrado</h3>
              <p className="text-sm text-[var(--ink-60)]">Adicione os profissionais que trabalham na sua empresa.</p>
              <p className="text-xs text-[var(--ink-30)] mt-1">Os agendamentos ainda podem ser criados sem profissional.</p>
            </div>
            <Button onClick={openCreate} className="bg-[var(--coral)] hover:bg-[var(--coral-dark)] text-white gap-2 rounded-xl mt-2">
              <UserPlus className="h-4 w-4" />Adicionar primeiro profissional
            </Button>
          </CardContent>
        </Card>
      ) : (
            ...
            <Card key={p.id} className="border-[var(--ink-10)] hover:border-[var(--coral)] transition-colors duration-200">
              ...
              <CardContent className="pt-0">
                <div className="flex gap-2">
                  ...
                  >
                    {p.available
                      ? <><XCircle className="h-3 w-3 text-[var(--ink-60)]" />Desativar</>
                      : <><CheckCircle className="h-3 w-3 text-[var(--success)]" />Ativar</>
                    }
                  </Button>
                  ...
                  <Button
                    size="sm"
                    variant="outline"
                    className="rounded-lg text-red-500 hover:bg-red-50 hover:border-red-200"
